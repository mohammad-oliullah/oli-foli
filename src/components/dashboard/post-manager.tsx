...
export function PostManager({ initialPosts }: { initialPosts: CmsPost[] }) {
  ...
  async function save() {
    ...
    if (!response.ok || !("_id" in payload)) {
      setError(
        "error" in payload
          ? payload.error || "Unable to save post."
          : "Unable to save post.",
      );
      return;
    }
    setPosts((current) =>
      editing
        ? current.map((post) => (post._id === payload._id ? payload : post))
        : [payload, ...current],
    );
    edit(payload);
  }
    ...
    <div className="grid gap-6 xl:grid-cols-[18rem_minmax(0,1fr)]">
      <aside className="rounded-lg border p-3 xl:sticky xl:top-4 xl:h-fit">
        ...
        <ul className="mt-3 space-y-1">
          {posts.map((post) => (
            <li key={post._id}>
              <button
                type="button"
                onClick={() => edit(post)}
                className="w-full rounded-md px-3 py-2 text-left text-sm hover:bg-muted focus-visible:outline-none focus-visible:ring-3 focus-visible:ring-ring/50"
              >
          ...
          {posts.length === 0 ? (
            <li className="px-3 py-5 text-sm text-muted-foreground">
              No posts yet.
            </li>
          ) : null}
        </ul>
      ...
      <section className="space-y-5 rounded-lg border p-4 sm:p-6">
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="font-mono text-[10px] uppercase tracking-wider text-muted-foreground">
              {editing ? "Edit post" : "New draft"}
            </p>
            <h2 className="mt-1 text-lg font-semibold">{previewTitle}</h2>
          </div>
          {editing ? (
            <button
              type="button"
        ...
        <div className="grid gap-5 lg:grid-cols-2">
          <Field label="Markdown content">
            <textarea
              ...
              rows={18}
              className="field min-h-96 resize-y font-mono text-xs leading-6"
            />
          </Field>
          <div>
            <p className="text-sm font-medium">Preview</p>
            <article className="prose-lite mt-2 min-h-96 rounded-md border p-4 text-sm leading-7">
              <ReactMarkdown>
                {draft.content || "Write Markdown to preview it here."}
              </ReactMarkdown>
            </article>
          </div>
        </div>
        ...
        >
          {isSaving ? "Saving…" : editing ? "Save changes" : "Create draft"}
        </button>
      </section>
    </div>
  );
